...
## Adding more depencies especially rust as it's needed form Clamav
apt-get install -y flex bison python3-dev pkg-config ninja-build rust-all gcc make python3-pytest valgrind check libbz2-dev libcurl4-openssl-dev libjson-c-dev libmilter-dev libncurses5-dev libpcre2-dev libssl-dev libxml2-dev zlib1g-dev
python3 -m pip install mussels

## Adding cmake via pip to have a recent verion
...
git clone --depth 1 https://github.com/Cisco-Talos/clamav-mussels-cookbook
...
## adding libclamav_rust
cp clamav/build/x86_64-unknown-linux-gnu/release/libclamav_rust.a ./lib

#cp "$CLAMAV_DEPENDENCIES/lib/libbz2_static.a" ./lib
cp "$CLAMAV_DEPENDENCIES/lib/libbz2.a" ./lib
